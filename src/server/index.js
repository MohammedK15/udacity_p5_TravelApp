...
app.post('/add-travel', async (req, res) => {
  ...
  await handleWeatherbitApi(
    travelInfo.travelCity,
    travelInfo.daysToStartTravel,
    travelInfo.travelStart,
    travelInfo.travelEnd
  ).then(async function (weatherbiteResponse) {
    await handleGeoNameApi(weatherbiteResponse).then(async function (geoNameResponse) {
...
const handleWeatherbitApi = async (city, daysToStart, startDate, endDate) => {
  ...
  if (daysToStart >= 7) {
    weatherbitApiUrl =
      WEATHERBIT_API_BASE_URL +
      'forecast/daily?city=' + city + '&start_date=' + startDate + '&end_date=' + endDate +
...
const handleGeoNameApi = async (data) => {
  ...
  const geoNameApiUrl =
    GEONAME_API_BASE_URL + '?lat=' + data.lat + '&lng=' + data.lon + '&username=' + GEONAME_API_KEY + '&type=JSON'
